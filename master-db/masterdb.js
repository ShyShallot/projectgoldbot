...
const masterdb = module.exports = {
  ...
  setup: async function(guildId, bot){
    try {
        ...
        command = `SELECT EXISTS (SELECT * FROM information_schema.tables WHERE table_schema = '${sqlConfig.db}' AND table_name = 'guild_config_${guildId}')`

        const [rows,fields] = await this.connection.execute(command)

        const existsValue = rows[0][Object.keys(rows[0])[0]]; // i love SQL SO FUCKING MUCH
